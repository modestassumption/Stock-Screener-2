    ...
    <div style={{ padding: '28px 32px', maxWidth: 1400, margin: '0 auto' }}>
      ...
      <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 16 }}>
          <Select label="Index" value={index} onChange={e => setIndex(e.target.value)}
            options={INDICES} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))',
          gap: 16, marginTop: 16 }}>
          <RangeSlider label="RSI Min" min={0} max={100} value={rsiMin} onChange={setRsiMin} />
          <RangeSlider label="RSI Max" min={0} max={100} value={rsiMax} onChange={setRsiMax} />
          <RangeSlider label="Min RS Rating" min={0} max={100} value={minRs} onChange={setMinRs} />
        </div>

        <div style={{ display: 'flex', gap: 24, marginTop: 16, alignItems: 'center',
          flexWrap: 'wrap', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', gap: 24 }}>
            <Toggle label="Above SMA 200" checked={above200} onChange={setAbove200} />
            <Toggle label="Above SMA 50"  checked={above50}  onChange={setAbove50} />
          </div>
          <Button onClick={handleRun} loading={loading} style={{ minWidth: 140 }}>
            ▶ Run Screener
          </Button>
        </div>
      </div>
      ...
      {!loading && data && (
        <>
          ...
          {results.length === 0
            ? <EmptyState icon="⊘" title="No stocks matched" sub="Try relaxing your filters." />
            : (
              <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)',
      ...
      {!loading && !data && (
        <EmptyState icon="⊞" title="Run the screener"
