...
class DataFetcher:
    ...
    def get_realtime_quote(self, ticker: str) -> dict:
        ...
        try:
            ...
            pc = getattr(fi, "previous_close", 0) or 0
            cur = getattr(fi, "last_price", 0) or 0
            if cur > 0:
                return {
                    "source": "yfinance", "price": cur,
                    "change": round(cur - pc, 2), "change_pct": round((cur - pc) / pc * 100, 2) if pc else 0,
                    "high": getattr(fi, "day_high", 0), "low": getattr(fi, "day_low", 0),
                    "open": getattr(fi, "open", 0), "prev_close": pc,
                }
        except Exception as e:
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                pc = float(df["Close"].iloc[-2]) if len(df) > 1 else cur
                return {
                    "source": "yfinance_hist", "price": cur,
                    "change": round(cur - pc, 2), "change_pct": round((cur - pc) / pc * 100, 2) if pc else 0,
                }
        except Exception as e:
            logger.warning(f"hist fallback failed for {ticker}: {e}")
        return {}
    ...
    def get_nse_tickers(self, index: str = "Nifty 50") -> List[str]:
        ...
        try:
            ...
            symbols = [
                ...
                if row.get("symbol") and " " not in row["symbol"] and row["symbol"] != index.replace(" ", "")
            ]
